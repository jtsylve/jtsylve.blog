...
- **Phase 1 (OMAP_REAP_PHASE_MAP_TREE):** The Reaper first removes entries whose object identifier is below `om_min_oid`, freeing their physical blocks back to the Space Manager. It then iterates the OMAP B-Tree and removes entries whose physical blocks are only referenced by the deleted snapshot, freeing those blocks as well. The Reaper processes entries in batches across multiple transactions, yielding between batches to allow other work to proceed.
...
typedef struct evict_mapping_val {
    ...
    uint64_t len;         // length of the relocated range
} evict_mapping_val_t;
```
